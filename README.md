...
1. 새 프로젝트는 SQL Editor에서 [supabase-schema.sql](./supabase-schema.sql)을 한 번 실행합니다.
   이미 운영 중인 프로젝트에는 전체 스키마 파일을 다시 실행하지 말고, 아직 적용하지 않은 migration만 시간순으로 실행합니다. 이번 카테고리 전환에는 [20260905-confidential-posts.sql](./supabase-migrations/20260905-confidential-posts.sql)을 아직 적용하지 않았다면 먼저 실행한 뒤, [20260907-editable-categories.sql](./supabase-migrations/20260907-editable-categories.sql)을 한 번 실행합니다.
